...
# -*- coding: utf-8 -*-
"""
This script is used to host the Socks4/5 server.
"""
...
def main(handlerPort, proxyPort, certificate, privateKey):
    _thread.start_new_thread(
        server, (handlerPort, proxyPort, certificate, privateKey))
    while True:
        time.sleep(60)
    ...
    try:
        ...
        while True:
            try:
                ...
                print("Reverse Socks Connection Received: {}:{}".format(
                    address[0], address[1]))
                try:
                    data = b""
                    while (data.count(b'\n') < 3):
...
def server(handlerPort, proxyPort, certificate, privateKey):
    q = queue.Queue()
    _thread.start_new_thread(
        handlerServer, (q, handlerPort, certificate, privateKey))
    try:
        ...
        while True:
            try:
                ...
                print("Socks Connection Received: {}:{}".format(
                    address[0], address[1]))
                client_socket = getActiveConnection(q)
                if client_socket == None:
                    client_socket2.close()
                _thread.start_new_thread(
                    forward, (client_socket, client_socket2))
                _thread.start_new_thread(
                    forward, (client_socket2, client_socket))
            except Exception as e:
                print(e)
                pass
    except Exception as e:
...
if __name__ == '__main__':
    if len(sys.argv) < 5:
        print("Usage:{} <handlerPort> <proxyPort> <certificate> <privateKey>".format(
            sys.argv[0]))
    else:
